        ...
        <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
          ...
          {!user ? (
            <button
              ...
              className="flex items-center gap-1.5 md:gap-2 px-3 md:px-5 py-2 md:py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 transition-all duration-200 hover:scale-105 text-xs md:text-sm font-semibold text-white"
            >
          ...
          ) : user.role !== 'admin' && subscriptionStatus?.hasSubscription && subscriptionStatus.subscription?.status === 'past_due' ? (
            <button
              onClick={() => navigate('/subscription')}
              className="flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-2 md:py-2.5 rounded-full bg-red-500/20 border border-red-500/40 transition-all duration-200 hover:scale-105 text-xs md:text-sm font-semibold text-red-400 animate-pulse"
              title="Payment failed — update your card"
            >
              <Crown className="w-3.5 h-3.5 md:w-4 md:h-4" strokeWidth={2} />
              <span className="hidden sm:inline">{t('subscription.paymentFailed', 'Payment Failed')}</span>
            </button>
          ) : user.role !== 'admin' && (
            <button
